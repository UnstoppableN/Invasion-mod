...
public class ItemSearingBow extends ItemBow
{
  ...
  {
    super();
    this.setUnlocalizedName("searingBow");
  ...
  {
      this.itemIcon = iconRegister.registerIcon("invmod:" + getUnlocalizedName().substring(5));
      this.iconArray = new IIcon[(bowPullIconNameArray.length+1)];
      this.iconArray[0]=iconRegister.registerIcon("invmod:" + getUnlocalizedName().substring(5));
      for (int i = 1; i < this.iconArray.length; ++i)
      {
          this.iconArray[i] = iconRegister.registerIcon("invmod:" + bowPullIconNameArray[i-1]);
      }
  }
  @Override
  @SideOnly(Side.CLIENT)
