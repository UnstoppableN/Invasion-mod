...
public class EntityAIAttackNexus extends EntityAIBase
{
  ...
  private boolean isNexusInRange()
  {
    ...
    for (int i = 0; i < size.getYCoord(); i++)
    {
      ...
      {
        if (this.theEntity.worldObj.getBlock(x + j, y, z - 1) == mod_Invasion.blockNexus)
        {
          if (isCorrectNexus(x + j, y, z - 1)) {
            return true;
          }
        }
        if (this.theEntity.worldObj.getBlock(x + j, y, z + 1 + size.getZCoord()) == mod_Invasion.blockNexus)
        {
      ...
      {
        if (this.theEntity.worldObj.getBlock(x - 1, y, z + j) == mod_Invasion.blockNexus)
        {
          if (isCorrectNexus(x - 1, y, z + j)) {
            return true;
          }
        }
        if (this.theEntity.worldObj.getBlock(x + 1 + size.getXCoord(), y, z + j) == mod_Invasion.blockNexus)
        {
    ...
    for (int i = 0; i < size.getXCoord(); i++)
    {
      for (int j = 0; j < size.getZCoord(); j++)
      {
        if (this.theEntity.worldObj.getBlock(x + i, y + 1 + size.getYCoord(), z + j) == mod_Invasion.blockNexus)
        {
          if (isCorrectNexus(x + i, y + 1 + size.getYCoord(), z + j)) {
            return true;
          }
        }
        if (this.theEntity.worldObj.getBlock(x + i, y - 1, z + j) == mod_Invasion.blockNexus)
        {
